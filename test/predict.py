        ...
        try:
            ...
            for prob_row in proba:
                ...
                if max_prob < CONFIDENCE_THRESHOLD:
                    ...
                    pred_labels.append("|".join(similar_labels))
...
def main():
    ...
    # 6. 使用所有模型进行预测
    predictions = predict_with_models(models_dict, X_test, class_names)
    ...
    for i, item in enumerate(result_data):
        for model_name, pred_labels in predictions.items():
            item[model_name] = pred_labels[i]
    
    # 构建表头
    headers = list(result_data[0].keys())
    
    # 保存所有模型的结果
    result_path = "result/res_all.csv"
    save_csv(result_data, headers, result_path)
    
    # 8. 保存注意力投票模型的预测结果
    print("\n保存注意力投票模型的预测结果...")
    
    if 'attention_voting_model' in predictions:
        attention_result_data = test_data.copy()
        for i, item in enumerate(attention_result_data):
            item['预测标签'] = predictions['attention_voting_model'][i]
        
        # 构建表头
        attention_headers = list(attention_result_data[0].keys())
        
        attention_result_path = "result/res_attention_vote.csv"
